  ...
  return (
    <div style={{ maxWidth: 640 }}>
      <div style={header}>
        ...
        <button onClick={() => navigate('/events')} style={btnBack}>← Voltar</button>
      </div>
      ...
      <form onSubmit={handleSubmit} style={formStyle}>
        <Field label="Título *">
          <input value={title} onChange={e => setTitle(e.target.value)} required maxLength={300} style={input} />
        </Field>
        <Field label="Descrição *">
          <textarea value={description} onChange={e => setDescription(e.target.value)} required rows={4} style={{ ...input, resize: 'vertical' }} />
        </Field>
        <Field label="Data e horário *">
          <input type="datetime-local" value={date} onChange={e => setDate(e.target.value)} required style={input} />
        </Field>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
          ...
          <button type="button" onClick={() => navigate('/events')} style={btnCancel}>Cancelar</button>
        </div>
      </form>
    </div>
  );
